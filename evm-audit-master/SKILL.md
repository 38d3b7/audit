...
| **Any external calls / callbacks / state mutation** (always) | `evm-audit-reentrancy` |
| **Any privileged role / owner / admin** (always) | `evm-audit-access-control` |
...
| Account abstraction (ERC-4337), smart wallets, paymasters, session keys | `evm-audit-erc4337` |
| EOA delegation, EIP-7702 set-code, sponsored txs via auth tuples | `evm-audit-eip7702` |
| Modular smart accounts (ERC-7579/6900), validator/executor modules, ERC-7484 registries | `evm-audit-modular-accounts` |
| Cross-chain bridges, LayerZero, CCIP, Wormhole, Across | `evm-audit-bridges` |
| Cross-chain intents, ERC-7683, solver/filler systems | `evm-audit-intents` |
...
### Baseline sources (Feb-2026 vintage)
...
### Refreshed sources (added v1.2.0)
- `[Solodit]` — solodit.cyfrin.io aggregated audit findings
- `[C4]` — Code4rena public contest reports (code4rena.com/reports)
- `[Sherlock]` — Sherlock contest reports (audits.sherlock.xyz/contests)
- `[Cantina]` — Cantina competition reports (cantina.xyz/competitions)
- `[Pashov]` — Pashov Audit Group published audits (github.com/pashov/audits)
- `[ToB]` — Trail of Bits Building Secure Contracts + Slither detector docs
- `[OZ 5.x]` — OpenZeppelin Contracts 5.x release notes & security advisories
- `[Cyfrin Updraft]` — Cyfrin Updraft curriculum (security/exploits modules)
- `[Immunefi]` — Immunefi disclosed bug bounty reports (immunefi.com/explore)
- `[EIP-7702]` — EIP-7702 spec + community implementation notes
- `[EIP-1153]` — EIP-1153 transient storage spec (Cancun)
- `[ERC-7579]` — ERC-7579 modular smart account spec + Rhinestone implementations
- `[ERC-6900]` — ERC-6900 modular account spec
